...
- Production Taoyuan paid-curb reference UI smoke after `npm run build`; the district-driven fixture verifies that a reviewed source row opens its representative point without changing the pinned address or producing a parking-legality answer, and that a boundary-excluded row remains text-only:
  ...
  Use `--reference-district guishan` for one district or `--all-reference-districts` to run the same safety contract against every published Taoyuan text and representative-point pack with one preview server.
...
- P3 reviewed release readiness for all reviewed/published districts:
  ...
  After a release is published, run `npm run ops:reconcile-release-evidence -- --package-url <zip-url> --manifest-url <manifest-url>` to validate that exact package, semantically verify every committed reviewed case, generate all release-district baselines in staging, and report repository evidence drift without modifying tracked files. Add `--execute` only after the report passes. The command writes only after every release district has both validated cases and a generated baseline, restores earlier files if a later write fails, and treats timestamps plus benchmark variance as unchanged when the dataset identity and deterministic baseline evidence are already aligned. Reports are written to `.tmp/release-evidence-reconcile.md/json`.
...
- Published release URL smoke:
  ...
  This checks the package URL with `HEAD`, falls back to `GET` with `Range: bytes=0-0` when `HEAD` is rejected, fetches the manifest URL, verifies the manifest `releaseId` matches the release being handed to Render, and rejects incomplete or duplicate per-district `datasetHash` plus `publishedAt` identities. The `Release Data Package` workflow runs it immediately after publishing GitHub Release assets.
- Live Render deployment verification:
  ...
  This reads the release manifest dataset identity contract, fetches `/api/parking-answer/ready`, and fails if the live service is missing a reviewed district, reports it as not ready, or serves a `datasetHash` or `publishedAt` different from the released package. Parking-answer readiness also initializes and queries the bundled spatial index; a runtime dependency failure appears under `invalidFiles` as `runtime/spatial-index` and prevents Render from marking the deploy healthy. Readiness uses a cold-start-aware 30-second timeout per attempt with three transient attempts by default; ordinary manifest and API probes retain the 15-second timeout. Override these independently with `--readiness-timeout-ms` and `--timeout-ms`. Comparing both identity fields prevents unchanged data content from making an older deployment look like the newly requested release. With `--all-parking-answer-cases`, it sequentially verifies every reviewed answer case; without the flag, local diagnostics use the first case per district. It also probes the live same-origin geocode, route, sync, and parking-answer health/ready endpoints, verifies geocoder/routing readiness exposes positive upstream request timeouts, runs a sync issue-report roundtrip, and verifies that sync CORS rejects an untrusted browser origin instead of returning wildcard access. If live dataset identities differ from the release contract, the markdown and JSON reports include a release-package remediation block with the exact Render package/manifest env vars, full build/redeploy steps, and follow-up verify command. If sync CORS or proxy timeout checks fail, the reports include a runtime remediation block with the exact Render env vars, redeploy steps, and follow-up verify command. The same check is available as the manual `Render Live Verify` GitHub Actions workflow by passing the Render app URL and published manifest URL; enable `useGithubToken` only for private GitHub Release asset URLs from this repository, leave `skipSyncIssueRoundtrip` false unless the live environment intentionally rejects sync smoke writes, and keep `allParkingAnswerCases` true for release acceptance. A dataset contract source is mandatory to prevent stale local state from being selected silently. When verifying locally from the same checkout that generated the handoff, explicitly pass `--handoff-json .tmp/render-deployment-handoff.json` instead of `--manifest-url`.
- Add `--require-durable-issue-sink` after D1 is connected. This requires both `issueSink.configured=true` in sync health and `durable=true, durability=external` from the live issue roundtrip. The manual workflow exposes the same contract as `requireDurableIssueSink`.
- Render release deployment wait:
  `npm run ops:wait-render-release -- --app-url https://<service>.onrender.com --handoff-json .tmp/render-deployment-handoff.json`
  This polls only the live readiness and exact per-district `datasetHash` plus `publishedAt` contract for up to 15 minutes. It writes `.tmp/render-release-wait.md/json` and fails closed on timeout. Use it after triggering a full Render deploy and before the complete live verification when the previous release may still be serving.
- Render runtime env sync:
  ...
  This previews the required production runtime hardening env vars and, when `--handoff-json` is present, the release package URL env vars from the reviewed handoff. Service-id dry-runs do not need a Render token. To apply, set `RENDER_API_KEY` and rerun with `--execute --deploy`; use `--service-name parkking` only when you need the tool to resolve the service ID through the Render API. You can also pass `--package-url` and `--manifest-url` explicitly. The same operation is available as the manual `Render Runtime Env Sync` GitHub Actions workflow when `RENDER_API_KEY` is configured as a repository secret. A successful `Release Data Package` run from the default branch or a `data-*` tag automatically starts the production path: it downloads that exact run's handoff artifact, syncs and deploys Render, waits for all released dataset identities, runs every reviewed live answer and Taoyuan reference UI smoke, and marks the data release Latest only after all gates pass. Configure `PARKKING_RENDER_SERVICE_ID` as a repository secret so the workflow can deploy without service-name lookup; optionally set the `PARKKING_RENDER_APP_URL` repository variable when the production URL is not `https://parkking.onrender.com`. Manual workflow dispatches remain dry-run by default and require `execute=true` to modify Render. Dispatch one from CLI with `npm run ops:render-runtime-env-sync-dispatch -- --repo <owner/repo> --ref main --handoff-json .tmp/render-deployment-handoff.json --dry-run`, then rerun without `--dry-run` and with `--execute` when `GH_TOKEN` or `GITHUB_TOKEN` is set.
...
4. Gate the reviewed CSV before ingest. This runs review summary, exports JSONL overrides, and preflights segment ids in one step:
   ...
   - Reviewed CSV exports require a valid review time from `reviewedHhmm` or the adjacent manifest's `params.hhmm`. The resulting schema-v2 override keeps the exact reviewed row in `reviewedSegmentId`, the normalized base id in `segmentId`, and the review time in `reviewedHhmm`.
   - A `reviewedSegmentId` ending in `-part-N` applies only to that evaluated part. A base-segment review can apply to its derived parts. The reviewed time applies only to the same parking time mode (day `07:00`-`20:00`, otherwise night), so a night review must not create an all-day rule.

...
- Render runtime env sync:
  ...
  This uses `RENDER_API_KEY` to resolve the Render service by name, update only the required production runtime hardening env vars directly on the service, then trigger a deploy when `--deploy` is present. Pass `--service-id <Render service ID>` when you already know the id, and omit `--execute` for a dry-run plan. Service-name dry-runs still need `RENDER_API_KEY` to resolve the id; service-id dry-runs do not. The same operation is available as the manual `Render Runtime Env Sync` GitHub Actions workflow when `RENDER_API_KEY` is configured as a repository secret; dispatch it from CLI with `npm run ops:render-runtime-env-sync-dispatch -- --repo <owner/repo> --ref main --dry-run`, then rerun without `--dry-run` when `GH_TOKEN` or `GITHUB_TOKEN` is set.

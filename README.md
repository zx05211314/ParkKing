...
- Render runtime env sync:
  ...
  This uses `RENDER_API_KEY` to update only the required production runtime hardening env vars directly on the Render service, then triggers a deploy when `--deploy` is present. Omit `--execute` for a dry-run plan.

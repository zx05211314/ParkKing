...
- Live Render deployment verification:
  ...
  This reads the release manifest dataset-hash contract, fetches `/api/parking-answer/ready`, and fails if the live service is missing a reviewed district, reports it as not ready, or serves a dataset hash different from the released package. It also probes the live same-origin geocode, route, sync, and parking-answer health/ready endpoints, verifies geocoder/routing readiness exposes positive upstream request timeouts, runs a sync issue-report roundtrip, and verifies that sync CORS rejects an untrusted browser origin instead of returning wildcard access. If sync CORS or proxy timeout checks fail, the markdown and JSON reports include a runtime remediation block with the exact Render env vars, redeploy steps, and follow-up verify command. The same check is available as the manual `Render Live Verify` GitHub Actions workflow by passing the Render app URL and published manifest URL; enable `useGithubToken` only for private GitHub Release asset URLs from this repository, and leave `skipSyncIssueRoundtrip` false unless the live environment intentionally rejects sync smoke writes. When verifying locally from the same checkout that generated the handoff, the command can use `.tmp/render-deployment-handoff.json` instead of `--manifest-url`.
- Render runtime env sync:
  `npm run ops:render-runtime-env-sync -- --service-name parkking --execute --deploy`
  This uses `RENDER_API_KEY` to resolve the Render service by name, update only the required production runtime hardening env vars directly on the service, then trigger a deploy when `--deploy` is present. Pass `--service-id <Render service ID>` when you already know the id, and omit `--execute` for a dry-run plan. Service-name dry-runs still need `RENDER_API_KEY` to resolve the id; service-id dry-runs do not. The same operation is available as the manual `Render Runtime Env Sync` GitHub Actions workflow when `RENDER_API_KEY` is configured as a repository secret; the workflow is dry-run by default and requires `execute=true` to modify Render. Dispatch it from CLI with `npm run ops:render-runtime-env-sync-dispatch -- --repo <owner/repo> --ref main --dry-run`, then rerun without `--dry-run` and with `--execute` when `GH_TOKEN` or `GITHUB_TOKEN` is set.

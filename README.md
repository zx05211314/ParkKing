...
- Release handoff status:
  ...
  This reads the local handoff artifacts, checks that copied local release assets still exist, checks whether the expected GitHub Release tag has already been published, and prints the exact local handoff, release dispatch, tag-push release workflow fallback, local handoff publish, Render env sync, and Render live-verify commands. Pass `--app-url` or set `PARKKING_RENDER_APP_URL` to render the final live verification command with a real Render service URL. When the release is already published, the first Render env sync command uses `--service-id "<Render service ID>"` so the handoff env plan can be previewed without resolving `parkking` through the Render API; the `--service-name parkking` variant remains available when `RENDER_API_KEY` is set. After `npm run ops:release-handoff-readiness`, use `npm run ops:release-data-publish-handoff -- --ref main --dry-run` to preview the token-based local GitHub Release upload, then rerun without `--dry-run` when `GH_TOKEN` or `GITHUB_TOKEN` is set. If local tokens and a logged-in GitHub UI are unavailable, push the printed `data-<release-id>` tag; the Release Data Package workflow derives `PARKKING_RELEASE_ID_INPUT` from that tag and publishes zip/manifest assets with filenames matching the handoff URLs.
- Release publish request handoff:
  `npm run ops:release-publish-request -- --ref main --app-url https://<service>.onrender.com`
  This writes `.tmp/release-publish-request.md` and `.tmp/release-publish-request.json` from the current handoff. The request includes the resolved target SHA, exact local zip/manifest paths, asset byte counts and SHA-256 checksums, token/CLI availability, a manual GitHub UI release-publish checklist, the exact local publish commands, the GitHub Actions dispatch and tag-push alternatives, Render environment variables, Render env sync service-id/service-name/workflow commands, and final verification commands. Use it when the code/data gates are ready but a human or another environment must supply GitHub/Render credentials.

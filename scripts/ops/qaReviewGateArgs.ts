import type { QaReviewGateArgs } from './qaReviewGateTypes'
...
export const parseQaReviewGateArgs = (argv: string[]): QaReviewGateArgs => {
  ...
  return {
    ...
    outDir: parseArgValue(argv, '--outDir'),
    outPath: parseArgValue(argv, '--out'),
    json: argv.includes('--json'),
    strictManifest: !argv.includes('--allow-manifest-warnings'),
    strictConfigProvenance: !argv.includes('--allow-config-provenance-warnings'),
    strictReviewedRows: !argv.includes('--allow-invalid-reviewed-rows'),
    strictReviewedSegments: !argv.includes('--allow-duplicate-reviewed-segments'),

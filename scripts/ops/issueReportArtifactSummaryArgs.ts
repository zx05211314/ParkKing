export interface IssueReportArtifactSummaryArgs {
  inputPath: string
  outPath: string | null
  label: string | null
...
export const parseIssueReportArtifactSummaryArgs = (
  argv: string[],
): IssueReportArtifactSummaryArgs => ({
  inputPath: normalizeRequiredText(
    parseAliasedArgValue(argv, '--input', '--index', 'input'),
    'input',
  ),
  outPath: normalizeOptionalText(parseArgValue(argv, '--out')),
  label: normalizeOptionalText(parseArgValue(argv, '--label')),
  inputUrl: parseAliasedArgValue(argv, '--input-url', '--index-url', 'input url'),
  ...
  writeIndexSummary: argv.includes('--write-index-summary'),
})

...
describe('renderBlueprintContract', () => {
  it('passes the checked-in Render blueprint', async () => {
    ...
    expect(result.checkedEnvVars).toContain('PARKKING_SYNC_MODE')
    expect(result.checkedEnvVars).toContain('PARKKING_SYNC_DURABILITY')
    expect(result.checkedEnvVars).toContain('VITE_SYNC_MODE')

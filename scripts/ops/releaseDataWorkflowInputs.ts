...
export interface ReleaseDataWorkflowInputs {
  ...
  releaseAllowAnswerCaseReviewFallback: string
...
export const resolveReleaseDataWorkflowInputs = (
  ...
  return {
    ...
    releaseAllowAnswerCaseReviewFallback: isDataTagPush
      ? 'true'
      : normalizeBooleanText(
          env.PARKKING_INPUT_ALLOW_ANSWER_CASE_REVIEW_FALLBACK,
        ),
...
const githubEnvLines = (inputs: ReleaseDataWorkflowInputs) => [
  ...
  `PARKKING_ALLOW_ANSWER_CASE_REVIEW_FALLBACK=${inputs.releaseAllowAnswerCaseReviewFallback}`,
